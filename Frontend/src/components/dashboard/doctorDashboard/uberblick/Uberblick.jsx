...
let todayAppsThisDoctor = todayApps.filter((item) => {return item.doctorId === doctor_id})
 ...
 let allAppointmentsThisDoctor = appointments.filter((item) => {return item.doctorId === userId})
 let receivedAppThisDoctor = allAppointmentsThisDoctor.filter((app) => {return app.patientId})
...
//.date 
    ...
    let patientsArray = allAppointmentsThisDoctor.filter((item) => {return item.patientId})
    //console.log(patientsArray)
    let patientsArray2 = []
    
    for(let i = 0; i < patientsArray.length - 1; i++){
        for(let j = i+1; j < patientsArray.length; j++){
            if(patientsArray[i]?.patientId.email !== patientsArray[j]?.patientId.email){
    ...
    const todayAppoThisDoctor = receivedAppThisDoctor.filter((element) => {return element.date === datumHeute})
        ...
        <div className='flex flex-row gap-8 w-[73%]'>
            <div className='flex flex-col max-w-[36rem]'>
                <div className='flex flex-row gap-8 border-b-[1.9rem] border-[#F1F7FE]'>
                    <div className='flex text-center flex-col justify-between bg-white p-3 max-h-[13rem] min-h-[13rem] max-w-[17rem] w-[16rem]'>
                        ...
                        <div className='flex justify-center items-center bg-[#F1F7FE] rounded-lg mx-6'>
                            ...
                            <p className='text-3xl text-[#38638D] font-bold pl-3'>{patientsArray2?.length}</p>
                        </div>
                    </div>
                    <div className='flex flex-col justify-between  text-center bg-white p-3 max-h-[13rem] min-h-[13rem] max-w-[17rem] w-[16rem]'>
                        ...
                        <div className='flex justify-center items-center bg-[#F1F7FE] rounded-lg mx-4'>
                            ...
                            <p className='text-3xl text-[#38638D] font-bold pl-3'>{receivedAppThisDoctor?.length}</p>
                        </div>
                    </div>
                </div>
            
                <div className='flex flex-col bg-white min-h-[62vh] max-w-[36rem]'>
            ...
            <div className='flex flex-col max-w-[36rem]'>
                <div className='flex flex-row gap-8 border-b-[1.9rem] border-[#F1F7FE]'>
                        ...
                        <div className='flex flex-col justify-between  text-center bg-white p-3 max-h-[13rem] min-h-[13rem] max-w-[17rem] w-[16rem]'>
                            ...
                            <p className='text-3xl text-[#38638D] font-bold pl-3'>{todayAppoThisDoctor?.length}</p>
                        </div>
                    </div>
                        
                    
                    <div className='flex flex-col justify-between  text-center bg-white p-3 max-h-[13rem] min-h-[13rem] max-w-[17rem] w-[16rem] rounded-tr-3xl'>
