...
const PatientInfo = ({
  patient,
  appsThisDoctor,
  appsThisDoctorSelectedDate,
  selectedDate,
}) => {
  //console.log(patient)
  let termin = [];
  if (patient) {
    termin = appsThisDoctor
      .filter((item) => item.patientId === patient)
    ...
    <div className="flex ml-4 flex-col justify-center items-center max-w-[22rem] text-center">
      {!appsThisDoctorSelectedDate.length ? (
        ...
        <div className="w-[20rem] text-center">
          <h1 className="mt-20 text-2xl">
            An diesem Tag haben Sie keine vereinbarten Termine.
          </h1>
        </div>
      ) : termin.length ? (
        <>
          <img
            src={patient?.profilePic || UserPNG}
            alt="Patient"
          ...
          {termin[0].isCancelled ? (
            <div className="w-[18vw] ml-3">
              {termin[0].cancelUserType === "patient" ? (
                <h1 className="text-red-600 mt-10 text-lg font-bold mx-auto">
        ...
        <div className="w-[23rem] text-center">
          <h1 className="mt-20 text-2xl">
            Bitte klicken Sie auf eine Patient-Name links um ihre/seine Info zu
            sehen.
          </h1>
        </div>
      )}
