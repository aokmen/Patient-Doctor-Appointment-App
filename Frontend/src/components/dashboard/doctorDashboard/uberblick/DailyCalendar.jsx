        ...
        <tbody>
          ...
          todayAppsThisDoctor?.map((item, index) => { 
            return <tr key={index}>
              ...
              <td onClick={()=>setPatient(item)} className={`flex items-center justify-center text-center p-1 ${item.patientId && 'hover:underline hover:cursor-pointer'} ${index % 2 === 0 ? 'bg-slate-300' : 'bg-slate-100'}`}><p className={`w-[15px] h-[15px] mr-2 rounded-full ${(item.patientId && !item.isCancelled) ? 'bg-green-500' : ((item.patientId && item.isCancelled) ? 'bg-red-600' : null) }`}></p>{!(item.patientId) ? "frei" : "besetzt" }</td>
            </tr>
          })
        }
        </tbody>
